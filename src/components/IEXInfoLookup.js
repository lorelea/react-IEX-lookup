...
class IEXInfoLookup extends React.Component {
  ...
  getAllSymbols() {
    APICall.getAllSymbols().then(response => {
      this.setState({
        symbols: response
      });
    });
  ...
  getLatestPrice() {
    APICall.getLatestPrice(this.state.activeSymbol).then(response => {
      this.setState({
        price: response
      });
    });
  ...
  getCompanyInfo() {
    APICall.getCompanyInfo(this.state.activeSymbol).then(response => {
      this.setState({
        company: response
      });
    });
  ...
  renderSymbolInfo() {
    ...
    return (
      <dl>
        ...
        <dd>
          {this.state.price} {symbolInfo.currency}
        </dd>
        <dt>Description</dt>
        <dd>{description}</dd>
      </dl>
  ...
  render() {
    const items =
      true || this.state.showAutocomplete
        ? this.state.symbols.map(item => item.symbol)
        : [];
    const options = this.state.symbols.map(item => ({
      value: item.symbol,
      label: item.symbol
    }));
    return (
      <div className="my-iex_info_lookup">
        ...
        />
        <Autocomplete
          className={this.state.showAutocomplete ? "" : "hide"}
          items={items}
        ...
        <WindowedSelect
          className={
            "my-iex_info_lookup-autocomplete " +
            (this.state.showAutocomplete ? "hide" : "")
          }
          filterOption={createFilter({ ignoreAccents: false })}
          isLoading={!options.length}
          options={options}
          value={options.find(
            option => option.value === this.state.activeSymbol
          )}
